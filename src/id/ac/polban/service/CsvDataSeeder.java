...
public final class CsvDataSeeder {

    private CsvDataSeeder() {}

    public static DataSeeder.Seed seedFromCsvs(String basePath) throws IOException {
    ...
    private static Map<String, Dosen> readDosenCsv(String basePath) throws IOException {
        ...
        for (int i = 1; i < lines.size(); i++) {
            ...
            try {
                ...
                if (parts.length < 3) {
                    System.err.println("Format baris tidak valid di dosen.csv (baris " + (i + 1) + "): " + line + ". Kolom tidak lengkap.");
                    continue;
                }
                Dosen d = new Dosen(parts[0].trim(), parts[1].trim(), parts[2].trim());
                dosenMap.put(d.getKodeDosen(), d);
            } catch (ArrayIndexOutOfBoundsException e) {
                System.err.println("Error parsing dosen.csv (baris " + (i + 1) + "): " + line + ". Pastikan format 'kode_dosen,nama,email'.");
            } catch (Exception e) {
                System.err.println("Error tidak terduga di dosen.csv (baris " + (i + 1) + "): " + line + ". Error: " + e.getMessage());
            }
        }
        return dosenMap;
    }
    ...
    private static Map<String, MataKuliah> readMataKuliahCsv(String basePath) throws IOException {
        ...
        for (int i = 1; i < lines.size(); i++) {
            ...
            try {
                String[] parts = line.split(",");
                if (parts.length < 3) {
                    System.err.println("Format baris tidak valid di matakuliah.csv (baris " + (i + 1) + "): " + line + ". Kolom tidak lengkap.");
                    continue;
                }
                ...
                System.err.println("Error parsing SKS di matakuliah.csv (baris " + (i + 1) + "): " + line + ". Pastikan SKS adalah angka.");
            } catch (ArrayIndexOutOfBoundsException e) {
                System.err.println("Error parsing matakuliah.csv (baris " + (i + 1) + "): " + line + ". Pastikan format 'kode_mk,nama_mk,sks'.");
            }
        }
        return mataKuliahMap;
    }

    private static Map<String, Kelas> readJadwalCsv(String basePath, Map<String, Dosen> dosenMap, Map<String, MataKuliah> mataKuliahMap) throws IOException {
        ...
        for (int i = 1; i < lines.size(); i++) {
            ...
            try {
                String[] parts = line.split(",");
                if (parts.length < 7) {
                    System.err.println("Format baris tidak valid di jadwal.csv (baris " + (i + 1) + "): " + line + ". Kolom tidak lengkap.");
                    continue;
                }
                ...
                if (mk == null || dosen == null) {
                    System.err.println("Data tidak valid di jadwal.csv (baris " + (i + 1) + "): " + line + ". Mata Kuliah atau Dosen tidak ditemukan.");
                    continue;
                }
                ...
                System.err.println("Error parsing waktu/hari di jadwal.csv (baris " + (i + 1) + "): " + line + ". Pastikan format hari (MONDAY) dan waktu (HH:mm) benar.");
            } catch (ArrayIndexOutOfBoundsException e) {
                System.err.println("Error parsing jadwal.csv (baris " + (i + 1) + "): " + line + ". Pastikan format 'kode_kelas,kode_mk,kode_dosen,hari,jam_mulai,jam_selesai,ruangan'.");
            } catch (Exception e) {
                System.err.println("Error tidak terduga di jadwal.csv (baris " + (i + 1) + "): " + line + ". Error: " + e.getMessage());
            }
        }
        return kelasMap;
    }
    ...
    private static List<Mahasiswa> readMahasiswaCsv(String basePath, Map<String, Kelas> kelasMap) throws IOException {
        ...
        for (int i = 1; i < lines.size(); i++) {
            ...
            try {
                String[] parts = line.split(",");
                if (parts.length < 3) {
                    System.err.println("Format baris tidak valid di mahasiswa.csv (baris " + (i + 1) + "): " + line + ". Kolom tidak lengkap.");
                    continue;
                }
                ...
                if (kelas != null) {
                    ...
                    System.err.println("Kelas dengan kode '" + kodeKelas + "' tidak ditemukan untuk mahasiswa " + nama + " (baris " + (i + 1) + "). Melewati penambahan ke kelas.");
                }
            } catch (ArrayIndexOutOfBoundsException e) {
                System.err.println("Error parsing mahasiswa.csv (baris " + (i + 1) + "): " + line + ". Pastikan format 'nim,nama,kode_kelas'.");
            } catch (Exception e) {
                System.err.println("Error tidak terduga di mahasiswa.csv (baris " + (i + 1) + "): " + line + ". Error: " + e.getMessage());
            }
        }
        return allMahasiswa;
    }
