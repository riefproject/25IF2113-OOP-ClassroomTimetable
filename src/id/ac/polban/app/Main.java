...
public class Main {
        ...
        case "2":
          {
            ...
            for (Kelas k : seed.getKelas()) {
              System.out.printf(
                  "- %s | %d mahasiswa | %d jadwal%n",
                  k.getKodeKelas(), k.getDaftarMahasiswa().size(), k.getDaftarJadwal().size());
            }
            break;
          }
        case "3":
  ...
  private static void printRingkasan(Seed seed) {
    ...
    for (Kelas k : seed.getKelas()) {
      System.out.println("-------------------------------------------");
      System.out.printf(
          "| %-10s | %-26d |%n", k.getKodeKelas(), k.getDaftarMahasiswa().size());
      ...
      System.out.println("    -------------------------------------------");
      k.getDaftarMahasiswa()
          .forEach(m -> System.out.printf("    | %-10s | %-26s |%n", m.getNim(), m.getNama()));
      System.out.println("    -------------------------------------------");
    }
    System.out.println("---------------------------------------------------- ");
    ...
    System.out.println(
        "-----------------------------------------------------------------------------------------------------------");
    System.out.printf(
        "| %-5s | %-10s | %-25s | %-30s | %-13s | %-5s |%n",
        "KELAS", "HARI", "MATKUL", "DOSEN", "WAKTU", "RUANG");
    System.out.println(
        "-----------------------------------------------------------------------------------------------------------");
    for (Kelas k : seed.getKelas()) {
      ...
      k.getDaftarJadwal() 
          .forEach(
              j -> {
                String waktu =
                    JadwalSearch.HM.format(j.getJamMulai())
                        + "-"
                        + JadwalSearch.HM.format(j.getJamSelesai());
                System.out.printf(
                    "| %-5s | %-10s | %-25s | %-30s | %-13s | %-5s |%n",
                    k.getKodeKelas(),
